...
In this problem we are given a server which generates RSA publickeys and are told that there are only 30 possible primes that the server uses to genereate them. First, we copy the public key `` and the cipher text
`` out of the `intercepted_message.pcap` using [Wireshark](https://www.wireshark.org/). This publickey must be a product of two of the 30 primes being used and the server lets us generate as many publickeys as we want and if we repeatedly request keys it wont be too long before one of the keys shares a prime with our key (See a python sample of this below). If this occurs, the GCD of the two keys will be the `p` component of the publickey and dividing the publickey, `N` by `p` will yeild the other prime `q`. Now that we have the two primes and know the encryption exponent, `e` equals 65537, we can easily compute the decryption key `d` as `` and the message as ``
...
Python Sample Code for factoring the publickey:
```python
